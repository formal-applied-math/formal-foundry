...
JUDGE_SYSTEM = (
    ...
    "SYMMETRICALLY — and this is the direction that actually goes wrong — DO fault a statement "
    "that ASSUMES MORE than the issue asks. A hypothesis the issue does not state, and that the "
    "conclusion does not need, makes the theorem strictly WEAKER; that is unfaithful in exactly "
    "the way a dropped fact is, and it is easy to miss because the statement looks careful. The "
    "recurring case is a GUARD ON A DIVISION: in Lean `x / 0 = 0`, so a quotient claim is usually "
    "already true in the degenerate case — `div_nonneg` needs only `0 ≤` on both legs and "
    "`mul_div_assoc` needs nothing at all. Before accepting a `≠ 0` / `0 <` side condition, ask "
    "whether the CONCLUSION actually fails without it; if the issue stated the goal without it, "
    "that is your answer. Do NOT be reassured by the proof using the hypothesis — a proof can "
    "consume a guard the theorem does not need. "
...
INTENT_SYSTEM = (
    ...
    "silently drop a fact you state — and never ADD a hypothesis the issue does not state, which "
    "weakens the theorem just as surely. If the issue states its goal without a side condition, "
    "specify it without one. "
...
FORMALIZE_SYSTEM = (
    ...
    "drop a hypothesis, or flip an inequality. EQUALLY: do NOT ADD a hypothesis the issue does "
    "not state. An extra assumption makes the theorem strictly weaker, which is the same failure "
    "as dropping a fact, and it is the one this pipeline actually commits.\n"
    "- NO GUARD ON A DIVISION unless the conclusion genuinely fails without it. In Lean "
    "`x / 0 = 0`, so a quotient claim is usually already true in the degenerate case: `div_nonneg` "
    "needs only `0 ≤` on both legs, and `mul_div_assoc` (`a * b / c = a * (b / c)`) needs nothing "
    "at all. Writing `(h : denom ≠ 0)` or `(h : 0 < denom)` by reflex — because in ordinary "
    "mathematics `x/0` is undefined — is the single most common way a draft ships a weaker "
    "theorem than was asked for. Test it: delete the hypothesis and check whether the CONCLUSION "
    "still holds, not whether your proof still works. Keep it only where the statement is false "
    "without it, as `1 ≤ a / b ↔ b ≤ a` needs `0 < b` to be comparable to 1 at all.\n"

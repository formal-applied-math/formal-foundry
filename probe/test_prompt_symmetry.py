...
def _stages():
    ...
    return [
        ("judge", P.JUDGE_SYSTEM),
        ("intent", P.INTENT_SYSTEM),
        ("formalize", P.FORMALIZE_SYSTEM),
    ]
...
def test_the_guidance_names_a_hypothesis_that_must_be_kept():
    ...
    assert "1 ≤ a / b" in P.FORMALIZE_SYSTEM or "comparable to 1" in P.FORMALIZE_SYSTEM
    assert "still required" in P.JUDGE_SYSTEM

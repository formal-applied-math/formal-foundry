...
a general model works out what the theorem should say, and how to break a hard one into
lemmas, while a Lean-specialised model writes the Lean and proves it. The second is to
...
A few choices shape it, and each gets a section. We keep the reasoning and the Lean in
separate models (\S\ref{sec:engines}). We check a statement for faithfulness before
...
We use two models. One is a general reasoning model (Magistral); the other, Leanstral, is
specialised for Lean~\cite{magistral2025}. Each takes the part of the work it handles
better.
...
an unbound universe, a binder in the wrong place. Deciding what to state, with which
hypotheses and at what generality, is a different skill from writing it down in Lean and
closing the goal, so we give the two to different models.

The reasoning model does three things: it drafts the \emph{intent} for a statement
(\S\ref{sec:drafter}), it splits a hard target into lemmas (\S\ref{sec:prove}), and it
judges whether a draft is faithful (\S\ref{sec:gates}). The Lean model does two: it turns
an intent into elaborating Lean, and it proves the result.
...
\begin{figure}[p]
  ...
  integration; stage~8 is the only human touchpoint. The reasoning model (Magistral) and
  the Lean model (Leanstral) recur across stages in the roles of \S\ref{sec:engines}.
...
and a little naming metadata), and writes no Lean. Second, \emph{formalise}: the Lean
model turns that into a single elaborating theorem, with a short repair loop that feeds
each elaboration error back for a fix. This is the two-model split of \S\ref{sec:engines},
now inside one draft.

\subsection{Grounding by retrieval}
Formalising well means knowing which declarations to name. We ground the Lean model with
...
pass. The Lean model likes to prepend an \lean{import} line, which is illegal partway
...
cheapest and safest to make. Reasoning and Lean go to the models trained for them. A
...
says the improvements worth making are in specification, which is where the two-model
split and the retrieval grounding are aimed.
...
The two models are Mistral's Magistral and a Lean-specialised model. The library itself is
linked above.
...
Mistral AI.
\newblock Magistral: reasoning models.

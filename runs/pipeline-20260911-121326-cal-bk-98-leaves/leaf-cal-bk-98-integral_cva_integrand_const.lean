...
-- apply: Real.exp_add, intervalIntegral.integral_congr, intervalIntegral.integral_const_mul, integral_exp, intervalIntegral.integral_comp_mul_left
theorem integral_cva_integrand_const (E₀ h₀ r T : ℝ) (hrh : r + h₀ ≠ 0) : ∫ u in (0:ℝ)..T, E₀ * Real.exp (-(r * u)) * h₀ * Real.exp (-(h₀ * u)) = E₀ * (h₀ / (r + h₀)) * (1 - Real.exp (-(r + h₀) * T)) := by sorry

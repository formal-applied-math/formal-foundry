...
-- apply: MathFin.hazardSurvival, MathFin.cumHazard_const, MathFin.cumHazard.eq_1, MathFin.hazardSurvival.eq_1, MathFin.survivalFromIntensity.eq_1
theorem hazardSurvival_const_eq_exp (h₀ u : ℝ) : hazardSurvival (fun _ : ℝ => h₀) u = Real.exp (-(h₀ * u)) := by sorry
